...
/**
 * Controller to support the scale maintenance status check of ePlum
 * transactions.
 * 
 * @author m314029
 * @since 2.18.3
 */
(function() {
	angular.module('productMaintenanceUiApp').controller('ScaleMaintenanceStoreDeptController', scaleMaintenanceStoreDeptController);


    scaleMaintenanceStoreDeptController.$inject = ['$stateParams', 'ngTableParams', 'ScaleMaintenanceApi', '$scope', '$interval', 'DownloadService', 'urlBase']; 

	/**
	 * Creates the controller for the scale maintenance loads.
	 */
	function scaleMaintenanceStoreDeptController($stateParams, ngTableParams, scaleMaintenanceApi, $scope, $interval, downloadService, urlBase){
		...
		/**
		 * Component ngOnInit lifecycle hook. This lifecycle is executed every
		 * time the component is initialized (or re-initialized).
		 */
		...
		/**
		 * Callback for a successful call to get list tracking data from the
		 * backend.
		 * 
		 * @param results
		...
		function loadData (results) {
			...
			self.error = null;
            firstSearch = false;
			// If this was the first page, it includes record count and total
			// pages .
			if (firstResult) { 
                firstResult = false;
		...
		/**
		 * Whether an Department has not been filled in, so user cannot add
		 * another Department.
		 * 
		 * @returns {boolean}
		 */
		/*
		 * self.isAnyDeptNull = function(){ return false; };
		 */
        ...
        /**
		 * Whether required Department statement information has been filled in
		 * or not.
		 * 
		 * @returns {boolean}
		 */
       ...
       /**
		...
		/**
		 * go to factory detail page, show more information for user. User an
		 * edit, add new on here.
		 * 
		 * @param tab
		 * @param item
		 */
