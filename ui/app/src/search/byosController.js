...
(function() {
		...
		function byosController(productSearchService, $rootScope, $scope, productSearchApi, productInfoApi) {
			...
			self.PENDING = "S    ";
			self.REJECTED = "R    ";
			// Search product by image change on date.
			self.IMAGE_CHANGE_ON_DATE = 22;
			...
			self.searchTypes = [
				...
				{type: self.IMAGE_CHANGE_ON_DATE, name: 'Image', operator: self.GT_LT_EQUAL, responseType:self.DATE},
            ...
            /**
			 * The Upc for image product search
             * @type {*|string}
             */
            self.UPC = productSearchService.UPC;
            /**
			 * The Item code for image product search.
             * @type {*|string}
             */
            self.ITEM_CODE = productSearchService.ITEM_CODE;
            /**
			 * The product id for image product search.
             * @type {*|string}
             */
            self.PRODUCT_ID = productSearchService.PRODUCT_ID;
            /**
             * The type of data the user has selected. For image search, this is UPC, product ID, or item code.
             *
             * @type {string}
             */
            self.selectedSearchType = productSearchService.UPC;
            /**
			 * Holds the upcs, product ids or item codes that needs to search for product by image type.
             * @type {string}
             */
            self.selectedSearchTypeValue = '';
			/**
			...
			self.userCanSearch = function() {
				...
				switch (self.selectedType.responseType) {
					...
					case self.DATE:
						// Check empty upc, product, item code for search product by image
						if(self.selectedType.type == self.IMAGE_CHANGE_ON_DATE) {
                            return !(self.selectedSearchTypeValue.trim().length == 0);
						}
			...
			self.getCustomSearchObject = function() {
				...
				if (self.selectedType.responseType == self.DATE) {
                    ...
                    var params = {
                        type: self.selectedType.type,
                        operator: self.ltGtEqOperand.id,
                        dateComparator: startDate,
                        endDateComparator: endDate,
                        description: message
                    };
					// Search product by upc, product id or item code.
                    if (self.selectedType.type == self.IMAGE_CHANGE_ON_DATE) {
                        if (self.selectedSearchType == productSearchService.ITEM_CODE) {
                            message = 'Item Codes: ' + self.selectedSearchTypeValue + ', '+ message;
                            params['itemCodes'] = self.selectedSearchTypeValue;
                        } else if (self.selectedSearchType == productSearchService.PRODUCT_ID) {
                            message = 'Product IDs: ' + self.selectedSearchTypeValue + ', '+ message;
                            params['productIds'] = self.selectedSearchTypeValue;
                        } else {
                            message = 'UPCs: ' + self.selectedSearchTypeValue + ', '+ message;
                            params['upcs'] = self.selectedSearchTypeValue;
                        }
            ...
            self.getBasicSearchTextPlaceHolder = function(){
                return 'Enter ' + self.selectedSearchType + 's to Search For';
            };
        }
	}
)();
