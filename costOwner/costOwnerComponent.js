...
(function () {
	...
	costOwnerController.$inject = ['$rootScope', '$scope', 'ngTableParams', 'costOwnerApi'];

	/**
	 * Constructs for cost owner Controller.
	 */
	function costOwnerController($rootScope, $scope, ngTableParams, costOwnerApi) {
		...
		/**
		 * Calls the api to update the cost owner.
		 */
		self.updateCostOwner = function () {
			...
			if (self.selectedRowIndex > -1) {
				...
				if (self.isCostOwnerChanged()) {
					if (self.validateCostOwnerBeforeUpdate()) {
						self.isWaitingForResponse = true;
		...
		self.addNewCostOwner = function () {
			...
			// Remove class ng-invalid in case Add new cost owner
			// after closing duplicate cost owner request from server in Add modal.
			$('#costOwnerName0').removeClass('ng-invalid');
		};
		...
		self.deleteRow = function (index) {
			...
			if (index % PAGE_SIZE === 0 && ((self.tableModalParams.page()-1)*PAGE_SIZE) === (self.newCostOwners.length) && self.tableModalParams.page() > 1) {
		...
		self.isValidNewCostOwner = function () {
			...
			for (var i = 0; i < self.newCostOwners.length; i++) {
				...
				if (self.isNullOrEmpty(self.newCostOwners[i].costOwnerName)) {
					...
					console.log("empty");
					self.newCostOwners[i].addClass = 'active-tooltip ng-invalid ng-touched';
				} else {
					...
					for (var k = i + 1; k < self.newCostOwners.length; k++) {
						if (self.newCostOwners[k].costOwnerName.toUpperCase() === self.newCostOwners[i].costOwnerName.toUpperCase()) {
							...
							self.newCostOwners[i].addClass = 'active-tooltip ng-invalid ng-touched';
							self.newCostOwners[k].addClass = 'active-tooltip ng-invalid ng-touched';
						}
					}
				}
			}
		 ...
		 * @returns {boolean}
		 */
		self.validateCostOwnerBeforeUpdate = function () {
		...
		/**
		 * Calls api to delete the cost owner.
		 */
		...
		self.initEmptyNewCostOwner = function () {
			...
			costOwner['costOwnerName'] = '';
			costOwner['topToTop'] = null;
			return costOwner;
		};
		...
		self.handleAddErrorMessage = function () {
			...
			for (var i = 1; i < tempMsgs.length; i++) {
				for (var j = 0; j < self.newCostOwners.length; j++) {
					if (self.newCostOwners[j].costOwnerName.toUpperCase() === tempMsgs[i].toUpperCase()) {
						self.newCostOwners[j].addClass = 'ng-invalid ng-touched';
					}
				}
			}
		};
		...
		/**
		 * Resets cost owner back to original state.
		 * @param index
		 */
		...
		/**
		 * Reset the status add or edit cost owner.
		 */ // Need to optimize this method
		self.resetSelectedCostOwner = function () {
			self.selectedRowIndex = -1;
			if (self.selectedCostOwner){
				self.selectedCostOwner.isEditing = false;
			}
			self.selectedCostOwner = null;
		};
		...
		/**
		 * Returns the disabled status of button by cost owner id.
		 *
		...
		/**
		 * Return the save new cost owners status.
		 *
		 * @returns {boolean}
		 */
